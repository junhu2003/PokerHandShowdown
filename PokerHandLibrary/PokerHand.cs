...
namespace PokerHandLibrary
{
    public class PokerHand : PokerPlayer
    {
        ...
        private Boolean CheckIsFlush()
        {
            ...
            if (getFlush.Count() == 1)
            {
                SuitType? _suit = getFlush?.DefaultIfEmpty()?.First()?.Suit;

                this.DominantSuit = _suit;
                this.IsFlush = true;
        ...
        private Boolean CheckIsThreeOfAKind()
        {
            ...
            {
                RankType? _rank = getThreeOfAKind?.DefaultIfEmpty()?.First()?.Rank;

                this.DominantRank = _rank;
        ...
        private Boolean CheckIsPair()
        {
            ...
            if (getPair.Count() >= 1)
            {
                RankType? _rank = getPair?.DefaultIfEmpty()?.First()?.Rank;
